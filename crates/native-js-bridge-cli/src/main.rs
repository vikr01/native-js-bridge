...
fn generate_bin_shim(root: &Path, scope: &str, binary: &str, platforms: &[PlatformEntry]) -> Result<()> {
    // Build the PACKAGES map entries.
    let mut packages_lines = String::new();
    for p in platforms {
        let name = platform_pkg_name(scope, binary, &p.os, &p.cpu);
        packages_lines.push_str(&format!(
            "  [`${{os}}-${{arch}}`]: {{ name: {name:?}, binExt: {bin_ext:?} }},\n",
            name = name,
            bin_ext = p.bin_ext,
        ));
    }
    // Replace the os/arch key template — we wrote it literally above; refactor to static map.
    // Actually build a static map keyed by "os-cpu" strings.
    let mut packages_map = String::new();
    for p in platforms {
        let key = format!("{}-{}", p.os, p.cpu);
        let name = platform_pkg_name(scope, binary, &p.os, &p.cpu);
        packages_map.push_str(&format!(
...
if (!entry) {{
  console.error(`{binary}: unsupported platform ${{process.platform}}-${{process.arch}}`);
  process.exit(1);
}}
...
try {{
  ...
  bin = path.join(pkgDir, "{binary}" + entry.binExt);
}} catch (e) {{
  console.error(`{binary}: platform package ${{entry.name}} is not installed`);
  process.exit(1);
}}
...
fn generate_platform_package_json(
    ...
    let name = platform_pkg_name(scope, binary, &p.os, &p.cpu);
    let dir = root.join("npm").join(format!("{binary}-{}-{}", p.os, p.cpu));
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    let root = cli.root.canonicalize().context("Failed to resolve --root")?;
    ...
    generate_bin_shim(&root, &meta.scope, &meta.binary, &meta.platforms)
        .context("bin shim")?;
